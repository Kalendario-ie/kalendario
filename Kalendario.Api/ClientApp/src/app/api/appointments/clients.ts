...
import {
    ...
    UpsertCustomerAppointmentRequest,
    UpsertEmployeeEventRequest
} from 'src/app/api/appointments/requests';
...
export const adminAppointmentClient: BaseModelRequest<Appointment, UpsertCustomerAppointmentRequest | UpsertEmployeeEventRequest> = {
    get: search => Promise.resolve({entities: []}),
    put: (id, command) => Promise.resolve(customerRequestAppointmentParser(null)),
    post: body => Promise.resolve(customerRequestAppointmentParser(null))
